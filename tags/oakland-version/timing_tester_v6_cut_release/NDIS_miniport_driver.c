...
//miniport driver, thus communicating with all the protocol level devices.
...
--*/
{
    ...
    do
    {
        ...
        //
		KdPrint(("__--__-___-MiniportAdapterHandle = %#x\n",MiniportAdapterHandle));
        ...
        //
        /// Don't want this in ours (VOID)PtRegisterDevice();
        ...
        if (NDIS_STATUS_SUCCESS!=Status)
          {
           KdPrint(("MPInitialize():  Failed inPassthruWMISetAddrArray(), status 0x%08x!\n", Status));
...
//  KdPrint(("MPSendPackets:  pAdapt = 0x%08x, MPSendPktsCt = %d\n", pAdapt, pAdapt->PassThruStats.MPSendPktsSeen));
    ...
    for (i = 0; i < NumberOfPackets; i++)
    {
        ...
        if (pAdapt->MPDeviceState > NdisDeviceStateD0)
        {
            NdisMSendComplete(ADAPT_MINIPORT_HANDLE(pAdapt),
                            Packet,
                            NDIS_STATUS_FAILURE);
			KdPrint(("Miniport is in low power state. Skipping packet\n"));
            continue;
        }
...
//KdPrint(("MPSendPackets():  In packet stacking1\n"));
					//KdPrint(("FilterPacket call 1\n"));
...
//........................................            // ja, 02.10.2003.
					...
					//KdPrint(("At MPSendPackets NdisSend1\n"));
...
//........................................            // ja, 02.10.2003.
            ...
            if (Status == NDIS_STATUS_SUCCESS)
            {
				...
				//KdPrint(("At MPSendPackets NdisSend2\n"));
                NdisSend(&Status,
                         pAdapt->BindingHandle,
                         MyPacket);
...
--*/
{
    ...
    do
    {
        ...
        if (Oid == OID_GEN_SUPPORTED_GUIDS)
          {                                           // ja, 28.09.2003.
           KdPrint(("MPQueryInformation OID_GEN_SUPPORTED_GUIDS called\n"));
           ...
           for (i = 0; i < LoopLim; i ++)             // Try several times to get current address array, with ever-increasing amounts of working storage.
             {
              if (0==i)                               // First try?
                ...
                {                                     // Use dynamically obtained storage.
                 ...
                 if (NDIS_STATUS_FAILURE==lclStatus)  // A problem?
                   {
                    KdPrint(("MPQueryInformation():  Failed to get memory for IP address array for adapter at 0x%08x!\n", pAdapt));
                    Status = NDIS_STATUS_RESOURCES;
                    *BytesNeeded = 0;
                    *BytesWritten = 0;
                    goto DoneOidQuery;
              ...
              if (i+1==LoopLim)                       // At loop limit?
                {
                 KdPrint(("MPQueryInformation():  Failed to get array last time, status 0x%08x!\n", lclStatus));
                 Status = NDIS_STATUS_FAILURE;
                 *BytesNeeded = 0;
                 *BytesWritten = 0;
                 goto DoneOidQuery;
...
//         KdPrint(("MPQueryInformation():  For adapter at 0x%08x, Oid = OID_CUSTOM_ARRAY needed = %d, given = %d\n",
...
--*/
{
    ...
    do
    {
		...
		//Possibly parse out the IP address assigned to this adapter
		if(Oid == OID_GEN_NETWORK_LAYER_ADDRESSES){
			KdPrint(("&&&&&&&&&&&&&&&&&&DID SEE OID_GEN_NETWORK_LAYER_ADDRESSES!\n"));
			KdPrint(("InformationBuffer = %#x\n", InformationBuffer));
			pNetAddrList = InformationBuffer;
			if(pNetAddrList->AddressCount != 0){

				if(pNetAddrList->AddressCount > 1){
					//I don't really know what to do with this case, but it has never been seen in the wild
					//In the future, it should be able to send some type of error message
					//to the central server so that I can look into it.
					//Maybe keep the IP that it came from as a fallback addr for these errors
					KdPrint(("Multiple IP addrs sent to miniport. Don't know what to do\n"));
					/////__asm{int 3}
				}
				//Else assume there's just one
				pNetAddr = pNetAddrList->Address;
				if(pNetAddr->AddressType != NDIS_PROTOCOL_ID_TCP_IP){
					KdPrint(("AddressType != NDIS_PROTOCOL_ID_TCP_IP\n"));
					/////__asm{int 3}
				}
				pIP = (PNETWORK_ADDRESS_IP)pNetAddr->Address;
				gSetInfoIP = (unsigned int)pIP->in_addr; //Network ordering?
				KdPrint(("set gSetInfoIP to %#x\n", gSetInfoIP));
			}
			else{
				//NOTE: This case can be caused by a "network cable disconnected" kind of event
				KdPrint(("Why does it even ever have 0 addresses?\n"));
				//__asm{int 3}
			}
		}
...
//         KdPrint(("MPSetInformation():  For adapter at 0x%08x, Oid = OID_CUSTOM_ARRAY\n",
//                   pAdapt));
           ...
           if (NDIS_STATUS_SUCCESS!=lclStatus)
             {
              KdPrint(("MPSetInformation():  Failed in PassthruWMISetAddrArray(), status 0x%08x!\n", lclStatus));
              Status = lclStatus;
              *BytesRead = 0;
              *BytesNeeded = 0;
              break;
...
--*/
{
	...
	//I don't think this ASSERT is sufficient, since it won't
	//exist in free build, and then it can lead to null
	//pointer deref?
	if(InformationBuffer == NULL) return;
...
--*/
{
...
#endif // NDIS51
    {
		...
		// Technically any NDisAllocateBuffer chains should be freed also but we don't do this at the moment.

		NdisQueryPacket(Packet, NULL, &bufCnt, NULL, NULL);
		if (bufCnt != 0)
			KdPrint(("********* MPReturnPacket: Gonna drop %d Buffers from 'Packet'!!!!\n", bufCnt));
		ASSERT(bufCnt == 0);

		NdisQueryPacket(MyPacket, NULL, &bufCnt, NULL, NULL);
		if (bufCnt != 0)
			KdPrint(("********* MPReturnPacket: Gonna drop %d Buffers from 'MyPacket'!!!!\n", bufCnt));
		ASSERT(bufCnt == 0);
...
--*/
{
    ...
    // was created.
    //
    /// Don't even reg the ioctl anymore //(VOID)PtDeregisterDevice();
